...
- Invite/redeem system with days + limit (supports unlimited)
- Logging
- Typed commands + button menus
"""

import os, json, requests, asyncio, imaplib, email, logging, random, string, subprocess
...
def check_sub(user_id):
    if str(user_id) in map(str, ADMIN_ID):  # admin bypass
...
async def myinfo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = load_db()
    if uid not in db:
        return await safe_reply(update, "❌ No subscription.")
    u = db[uid]
...
async def list_emails(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = load_db()
    emails = db.get(uid, {}).get("emails", [])
    if not emails:
        return await safe_reply(update, "No emails created.")
...
async def delete_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /delete username")
    ...
    if email_addr not in owned:
        return await safe_reply(update, "⛔ You don't own this email.")

    url = f"{CPANEL_HOST}/execute/Email/delete_pop"
    data = {"domain": DOMAIN, "email": username}

    try:
        r = requests.post(url, headers=cpanel_headers(), data=data, verify=False).json()
        if r.get("status") == 1:
            db[uid]["emails"] = [m for m in db[uid]["emails"] if
                                (m["address"] if isinstance(m, dict) else m) != email_addr]
            save_db(db)
            log_action(uid, f"Deleted {email_addr}")
            await safe_reply(update, f"🗑 Deleted {email_addr}")
        else:
            await safe_reply(update, "❌ Error deleting email.")
    except Exception as e:
        await safe_reply(update, f"❌ Exception: {e}")

async def reset_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        return await safe_reply(update, "Usage: /password username newpass")

    username, newpass = context.args
    url = f"{CPANEL_HOST}/execute/Email/passwd_pop"
    ...
    try:
        ...
        if r.get("status") == 1:
            ...
            await safe_reply(update, "❌ Error resetting password.")
    except Exception as e:
        await safe_reply(update, f"❌ Exception: {e}")

async def set_defaultpw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /defaultpassword newpass")
    uid = str(update.effective_user.id)
    set_user_password(uid, context.args[0])
    await safe_reply(update, "✅ Default password updated.")

async def show_defaultpw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    await safe_reply(update, f"🔑 Default password: {get_user_password(uid)}")
...
async def inbox(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /inbox username")

    username = context.args[0]
    email_addr = f"{username}@{DOMAIN}"
    pw = get_user_password(update.effective_user.id)
    ...
    try:
        ...
        ids = data_ids[0].split()[-5:]

        if not ids:
            return await safe_reply(update, "📭 No mails found.")

        for num in ids:
            _, msg_data = mail.fetch(num, "(RFC822)")
            msg = email.message_from_bytes(msg_data[0][1])
            body = extract_body(msg)
            preview = f"📩 Mail in {email_addr}\nFrom: {msg['from']}\nSubject: {msg['subject']}\n---\n{body[:3000]}"
            await safe_reply(update, preview)
        mail.logout()
    except Exception as e:
        await safe_reply(update, f"❌ Inbox error: {e}")
...
async def add_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 3:
        return await safe_reply(update, "Usage: /addsub user_id days limit")
    ...
    save_db(db)
    log_action(update.effective_user.id, f"Added sub for {user_id}")
...
async def update_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        result1 = subprocess.run(["git", "fetch", "--all"], capture_output=True, text=True)
        result2 = subprocess.run(["git", "reset", "--hard", "origin/main"], capture_output=True, text=True)
...
async def reset_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /reset user_id")
    ...
    if user_id in db:
        db[user_id]["used"] = 0
        save_db(db)
        log_action(update.effective_user.id, f"Reset {user_id}")
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /ban user_id")

    user_id = context.args[0]
    db = load_db()
    db[user_id] = {"expiry": "1970-01-01T00:00:00", "limit": 0, "used": 0, "emails": []}
    save_db(db)
    log_action(update.effective_user.id, f"Banned {user_id}")
...
async def suspend_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /suspend user_id")

    user_id = context.args[0]
    db = load_db()
    if user_id in db:
        db[user_id]["suspended"] = True
        save_db(db)
        log_action(update.effective_user.id, f"Suspended {user_id}")
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    db = load_db()
    total_users = sum(1 for uid, u in db.items() if isinstance(u, dict) and uid not in map(str, ADMIN_ID))
    total_user_emails = sum(u.get("used", 0) for uid, u in db.items() if isinstance(u, dict) and uid not in map(str, ADMIN_ID))
    admin_emails = sum(db.get(str(a), {}).get("used", 0) for a in ADMIN_ID)
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, info in db.items():
        ...
        emails = [m["address"] if isinstance(m, dict) else m for m in info.get("emails", [])]
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 1:
        return await safe_reply(update, "Usage: /broadcast msg")
...
async def quota(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        return await safe_reply(update, "Usage: /quota username MB")
...
async def invite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    days, limit = int(context.args[0]), int(context.args[1])
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code in db.get("invites", {}):
        ...
        db[uid] = {"expiry": expiry, "limit": limit, "used": 0, "emails": [], "default_password": generate_password()}
        save_db(db)
...
def main():
    ...
    logging.info("🚀 Nervy Mailbot started...")
    app.run_polling()
